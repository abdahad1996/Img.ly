...
public struct RootView: View {
  ...
  public var body: some View {
    if isActive {
      makeTreeFlowView(
        designLibrary: isToggled
          ? DesignLibraryAlternative() : DesignLibrary(),
    ...
    NavigationStack(path: $treeflow.path) {
      ...
      .navigationBarItems(
        leading:
          ...
          .toggleStyle(
            SwitchToggleStyle(
              tint: designLibrary.color.background.buttonPrimary))
      )
  ...
  @ViewBuilder public func makeLeafView(
    id: String, designLibrary: DesignLibraryProvider
  )
    -> some View
  {
    //remote loader
    let remoteLeafNodeLoader = LeafFactory.remoteLeafNodeLoader(
    ...
    let leafNodeLoaderCacheDecorator = LeafNodeLoaderCacheDecorator(
      ...
      leafNodeCache: localLeafNodeLoader)

    let leafNodeLoaderWithFallbackComposite =
      LeafNodeLoaderWithFallbackComposite(
        primaryLoader: leafNodeLoaderCacheDecorator,
        secondaryLoader: localLeafNodeLoader)
