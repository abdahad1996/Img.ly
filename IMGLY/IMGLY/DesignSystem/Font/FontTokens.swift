...
public struct CardFont: CardFontProvider {
    ...
    public let body = Font.custom(
        CustomFont.spaceGroteskRegular.rawValue, size: 16)
    public let price = Font.custom(CustomFont.heptaSlabBold.rawValue, size: 24)
}

public struct CardFontAlternative: CardFontProvider {
    public let title = Font.custom(
        CustomFont.playfairDisplayBold.rawValue, size: 36)
    public let body = Font.custom(CustomFont.ralewayRegular.rawValue, size: 16)
    public let price = Font.custom(
        CustomFont.playfairDisplayBold.rawValue, size: 24)
}
...
public struct ButtonFont: ButtonFontProvider {
    public let standard = Font.custom(
        CustomFont.spaceGroteskBold.rawValue, size: 16)
}
