...
struct LeafView: View {
	...
	var body: some View {
		...
		.navigationBar(backgroundColor: .clear, titleColor: designLibrary.color.text.standard)
	...
	@ViewBuilder fileprivate func detailText(_ title: String, _ nodeText: String) -> some View {
		HStack {
			Text("\(title)").font(designLibrary.font.cardFont.body)
			Text(" \(nodeText.formattedDate())").font(designLibrary.font.cardFont.body)
...
#Preview(traits: .sizeThatFitsLayout) {
    LeafView(leafViewModel: LeafViewModel(loader: stubbedLeafLoader(), id: "2"), designLibrary: DesignLibrary())
}
