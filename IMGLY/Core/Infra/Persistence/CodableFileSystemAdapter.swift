...
public class CodableFileSystemAdapter: LocalStore {
    ...
    public struct CodableLeafNode: Codable {
        ...
        var localLeafNode: LocalLeafNode {
            return LocalLeafNode(
                id: id, createdAt: createdAt, createdBy: createdBy,
    ...
    fileprivate func saveFirstTime(
        _ object: LocalLeafNode, _ timestamp: Date, _ id: String
    ) throws {
        let encoder = JSONEncoder()
        let cache = Cache(
            codableLeafNode: CodableLeafNode(node: object),
    ...
    public func write(_ id: String, _ object: LocalLeafNode, timestamp: Date)
        async throws
    {
        do {
            guard let data = try? Data(contentsOf: storeURL) else {
