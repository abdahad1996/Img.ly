...
public enum RemoteLeafMapper {
    ...
    private static func mapToLeafNode(node: RemoteLeafNode) -> LeafNode {
        return LeafNode(
            id: node.id, createdAt: node.createdAt, createdBy: node.createdBy,
            lastModifiedAt: node.lastModifiedAt,
            lastModifiedBy: node.lastModifiedBy, description: node.description)
    }

    private static var OK_200: Int { return 200 }
    public static func mapToLeafNode(from data: Data, response: HTTPURLResponse)
        throws -> LeafNode
    {
        guard response.statusCode == OK_200,
            let remoteLeafNode = try? JSONDecoder().decode(
                RemoteLeafNode.self, from: data)
        else {
