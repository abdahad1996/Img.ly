...
class RemoteLeafNodeLoaderTests:XCTestCase{
    ...
    func test_load_checksUrlConstruction() async {
        ...
        XCTAssertEqual(client.requestedURLs[0].absoluteString, "https://ubique.img.ly/frontend-tha/entries/imgly.B.1.json")
    }
...
//    
    func test_load_deliversConnectivityErrorOnClientError() async throws {
        ...
        do {
            ...
            XCTFail("Expected result \(RemoteLeafNodeLoader.Error.connectivity) got \(result) instead")
...
//    
    func test_load_deliversInvalidDataErrorOnNon200HTTPResponse() async throws {
        ...
        for code in samples {
            ...
            do {
                let result = try await sut.load(id:"")
                XCTFail("Expected result \(RemoteLeafNodeLoader.Error.invalidData) got \(result) instead")
            } catch let error as RemoteLeafNodeLoader.Error {
    ...
    func test_load_deliversSuccessWithLeafNodeOn200HTTPResponseWithJSONItems() async throws {
    ...
    private func makeSUT(url: URL = URL(string: "https://a-url.com")!, result: (Data, HTTPURLResponse)? = nil, error: RemoteLeafNodeLoader.Error? = nil, file: StaticString = #filePath, line: UInt = #line) -> (sut: RemoteLeafNodeLoader, client: HTTPClientSpy) {
    ...
    class HTTPClientSpy: HTTPClient {
        ...
        init(result: (Data, HTTPURLResponse)?, error: RemoteLeafNodeLoader.Error?) {
            self.result = result
            self.error = error
        }
