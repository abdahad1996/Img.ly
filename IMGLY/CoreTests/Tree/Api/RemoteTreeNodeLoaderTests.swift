...
class RemoteTreeNodeLoaderTests: XCTestCase {
    ...
    func test_load_deliversConnectivityErrorOnClientError() async throws {
        ...
        do {
            ...
            XCTFail(
                "Expected result \(RemoteTreeNodeLoader.Error.connectivity) got \(result) instead"
            )
    ...
    func test_load_deliversInvalidDataErrorOnNon200HTTPResponse() async throws {
        ...
        for code in samples {
            ...
            do {
                let result = try await sut.load()
                XCTFail(
                    "Expected result \(RemoteTreeNodeLoader.Error.invalidData) got \(result) instead"
                )
            } catch let error as RemoteTreeNodeLoader.Error {
    ...
    func
        test_load_deliversSuccessWithNoItemsOn200HTTPResponseWithEmptyJSONList()
        async throws
    {
    ...
    func test_load_deliversSuccessWithTreeNodesOn200HTTPResponseWithJSONItems()
        async throws
    {
    ...
    private func makeSUT(
        ...
        error: RemoteTreeNodeLoader.Error? = nil,
        file: StaticString = #filePath, line: UInt = #line
    ) -> (sut: RemoteTreeNodeLoader, client: HTTPClientSpy) {
    ...
    class HTTPClientSpy: HTTPClient {
        ...
        init(
            result: (Data, HTTPURLResponse)?, error: RemoteTreeNodeLoader.Error?
        ) {
            self.result = result
            self.error = error
        }
